...
public class GlobalAdapters {
    ...
    public Wallet toWallet(List<BitgetBalanceDto> bitgetBalanceDtos) {
        List<Balance> balances =
                bitgetBalanceDtos.stream().map(GlobalAdapters::toBalance).collect(Collectors.toList());
    ...
    public BitgetPlaceOrderDto toBitgetPlaceOrderDto(LimitOrder limitOrder) {
        BitgetPlaceOrderDto.TimeInForce timeInForce = getOrderFlag(limitOrder, BitgetPlaceOrderDto.TimeInForce.class).orElse(BitgetPlaceOrderDto.TimeInForce.GOOD_TIL_CANCELLED);
        return BitgetPlaceOrderDto.builder()
    ...
    public static BitgetCancelOrderDto toBitgetCancelOrderDto(BitgetCancelOrderParam params) {
        ...
        BitgetCancelOrderDto.BitgetCancelOrderDtoBuilder bitgetCancelOrderDtoBuilder =
                StringUtils.isNotBlank(params.getOrderId())
    ...
    public static OpenOrders toOpenOrders(List<BitgetOrderInfoDto> bitgetOrderInfoDtos) {
        ...
        bitgetOrderInfoDtos.forEach(
                dto -> {
                    if (dto.getOrderType().equals(BitgetOrderInfoDto.OrderType.LIMIT)) {
                        LimitOrder.Builder builder = new LimitOrder.Builder(dto.getOrderSide(), guessSymbol(dto.getSymbol()));
                        BigDecimal price = dto.getPrice() == null ? dto.getPriceAvg() : dto.getPrice();
                        LimitOrder limitOrder = builder.limitPrice(price)
                                .timestamp(DateUtil.date(dto.getUpdatedAt()))
                                .originalAmount(dto.getSize())
                                .cumulativeAmount(dto.getBaseVolume())
                                .remainingAmount(dto.getSize().subtract(dto.getQuoteVolume()))
                                .id(dto.getOrderId())
                                .userReference(dto.getClientOid())
                                .averagePrice(dto.getPriceAvg())
                                .orderStatus(OrderStatusConvert.convert(dto.getOrderStatus().name(), ExchangeCode.BITGET))
                                .fee(dto.getFee()).build();
                        limitOrders.add(limitOrder);
                    } else {
                        MarketOrder.Builder builder = new MarketOrder.Builder(dto.getOrderSide(), guessSymbol(dto.getSymbol()));
                        MarketOrder order = builder.timestamp(DateUtil.date(dto.getUpdatedAt()))
                                .originalAmount(dto.getSize().multiply(dto.getPrice()))
                                .id(dto.getOrderId())
                                .userReference(dto.getClientOid())
                                .averagePrice(dto.getPriceAvg())
                                .orderStatus(OrderStatusConvert.convert(dto.getOrderStatus().name(), ExchangeCode.BITGET))
