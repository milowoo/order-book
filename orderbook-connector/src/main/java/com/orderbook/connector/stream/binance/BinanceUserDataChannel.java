...
/**
 * Binance user data streams must be established by first requesting a unique "listen key" via authenticated
 * REST API, which is then used to create an obscured WS URI (rather than authenticating the web socket).
 * This class handles the initial request for a listen key, but also the 30-minute keepalive REST calls
 * necessary to keep the socket open. It also allows for the possibility that extended downtime might cause
 * the listen key to expire without being able to renew it. In this case, a new listen key is requested and a
 * caller can be alerted via asynchronous callback to re-establish the socket with the new listen key.
 *
...
class BinanceUserDataChannel implements AutoCloseable {
    ...
    /**
     * Creates the channel, establishing a listen key (immediately available from #getListenKey()) and starting
     * timers to ensure the channel is kept alive.
     *
    ...
    /**
     * Returns: The current listen key.
     *
     * Throws: {@link NoActiveChannelException} - If no listen key is currently available.
     */
    ...
    /**
     * Thrown on calls to getListenKey() if no channel is currently open.
     *
     * Author: Graham Crockford
     */
    static final class NoActiveChannelException extends Exception {
        private static final long serialVersionUID = -8161903286845820286L;
