...
public class BybitUserDataStreamingService extends JsonNettyStreamingService {
    ...
    public boolean waitForInitSuccess(long timeoutMillis) {
        if (isAuthorized) {
            return true;
        }
    ...
    private void login() {
        ...
        try {
            ...
            final SecretKey secretKey = new SecretKeySpec(spec.getSecretKey().getBytes(StandardCharsets.UTF_8), BaseParamsDigest.HMAC_SHA_256);
            mac.init(secretKey);
            String signature = bytesToHex(mac.doFinal(_val.getBytes(StandardCharsets.UTF_8)));
            List<String> args = Stream.of(key, String.valueOf(expires), signature).collect(Collectors.toList());
    ...
    protected WebSocketClientHandler getWebSocketClientHandler(WebSocketClientHandshaker handshake, WebSocketClientHandler.WebSocketMessageHandler handler) {
        return new BybitUserDataWebSocketClientHandler(handshake, handler);
    }

    class BybitUserDataWebSocketClientHandler extends NettyWebSocketClientHandler {
        public BybitUserDataWebSocketClientHandler(WebSocketClientHandshaker handshake, WebSocketClientHandler.WebSocketMessageHandler handler) {
