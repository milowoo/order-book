...
@Slf4j
public class GlobalStreamingService extends NettyStreamingService<BitgetWsNotification> {
    ...
    public GlobalStreamingService(String apiUri) {
        super(apiUri, Integer.MAX_VALUE);
    }

    @Override
    protected String getChannelNameFromMessage(BitgetWsNotification message) {
        return GlobalStreamingAdapters.toSubscriptionId(message.getChannel());
    }
    ...
    public String getSubscribeMessage(String channelName, Object... args) throws IOException {
        BitgetChannel bitgetChannel = GlobalStreamingAdapters.toGlobalChannel(args);
    ...
    protected void handleMessage(BitgetWsNotification message) {
        ...
        if (message instanceof BitgetEventNotification) {
            if (((BitgetEventNotification) message).getEvent().equals(BitgetEventNotification.Event.ERROR)) {
                // todo 是否考虑重连
                log.error("GlobalStreamingService received BitgetEventNotification:{}", JSONUtil.toJsonStr(message));
            }
            return;
        }
    ...
    public String getSubscriptionUniqueId(String channelName, Object... args) {
        BitgetChannel bitgetChannel = GlobalStreamingAdapters.toGlobalChannel(args);
        return GlobalStreamingAdapters.toSubscriptionId(bitgetChannel);
    }
    ...
    public void messageHandler(String message) {
        ...
        try {
            ...
            if (jsonNode.has("arg") && jsonNode.get("arg").has("coin")) {
                ((ObjectNode) jsonNode.get("arg")).put("instId", jsonNode.get("arg").get("coin").asText());
            }
